-- Correctif audit du 31/07 (point non bloquant #62) : jusqu'ici QualioFlex ne
-- savait que GÉNÉRER les documents (contenu_html). Certains formateurs ont déjà
-- leur convention/devis (rédigés hors QualioFlex, ou déjà signés) — on leur
...
comment on column public.documents_formation.fichier_url is
  'URL du fichier importé tel quel par le formateur (convention/devis déjà existants), en alternative à contenu_html (document généré automatiquement par QualioFlex). Utilisé quand la formation est en document_mode = ''import''.';
