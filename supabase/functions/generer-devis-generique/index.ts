...
// Le devis GÉNÉRIQUE est rattaché uniquement à la FORMATION (formation_id),
// sans session_id ni client_id — il sert au formateur pour la prospection,
// avant même qu'un client ou une session existent. Contrairement au devis de
// session (generer-devis), il ne contient donc aucune info client ni dates
// figées. Un seul devis générique par formation (type "devis_generique"),
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("generer-devis-generique: démarrage pour formation_id =", formation_id);
    ...
    const { data: formation, error: fErr } = await supabase
      .from("formations")
      .select("*, organismes(raison_sociale, nda, siret, adresse, telephone, email_contact, logo_url)")
      .eq("id", formation_id)
      .single();

    if (fErr || !formation) throw new Error("Formation introuvable : " + fErr?.message);
    const f = formation as Record<string, unknown>;
    const org = (f.organismes as Record<string, string>) || {};

    const titre = String(f.titre || "");
    const objectifs = String(f.objectifs || "");
    const duree = String(f.duree || "");
    const modalites = String(f.modalites || "");
    const tarif = String(f.tarif || "").trim();

    let objetTexte = `Formation « ${titre} » — ${objectifs || "voir programme détaillé"}.`;
    try {
      const prompt = `Rédige UNE SEULE phrase commerciale courte (30 mots maximum), en français, professionnelle, décrivant l'objet d'un devis de formation professionnelle intitulée "${titre}", destiné à un prospect. Objectifs pédagogiques : ${objectifs || "non précisés"}. Réponds UNIQUEMENT avec la phrase, sans guillemets, sans markdown.`;
      ...
      if (claudeRes.ok) {
        ...
        console.error("generer-devis-generique: Claude API non-ok, fallback utilisé:", claudeRes.status);
      }
    } catch (aiErr) {
      console.error("generer-devis-generique: échec appel Claude, fallback utilisé:", aiErr);
    }

    const dateDevis = new Date().toLocaleDateString("fr-FR");
    const numeroDevis = `DEV-GEN-${String(formation_id).slice(0, 8).toUpperCase()}`;

    const html = `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<title>Devis — ${esc(titre)}</title>
<style>
  ...
  .prospection { background: #eef1ff; border: 1px solid #c7d0f5; border-radius: 6px; padding: 10px 14px; font-size: 9.5pt; color: #25245e; margin-top: 14px; }
...
<body>
  ...
  <div class="bloc-client">
    <p><strong>Client :</strong> à compléter</p>
  </div>

  <h2>Objet</h2>
  <p class="objet">${esc(objetTexte)}</p>
  <p class="objet">Durée : ${esc(duree || "non précisée")} — Dates : à définir selon vos disponibilités — Modalités : ${esc(modalites || "non précisées")}.</p>

  <h2>Détail de la prestation</h2>
  <table>
    <thead>
      <tr><th>Désignation</th><th>Tarif</th></tr>
    </thead>
    <tbody>
      <tr>
        <td>${esc(titre)}</td>
  ...
  ${!tarif ? `<div class="a-verifier">⚠️ Aucun tarif n'est renseigné sur la fiche formation — complétez-le avant l'envoi.</div>` : ""}

  <div class="prospection">📋 Devis générique de prospection — à personnaliser (nom du client, dates) avant envoi définitif.</div>
  ...
  <div class="signature-zone">
    ...
    <div class="signature-box"><strong>Le client</strong><br/>Bon pour accord — Date et signature</div>
  </div>
  ...
  <div class="footer">Document généré par QalioFlex — ${esc(org.raison_sociale || "")}</div>
    ...
    const { data: existingDoc } = await supabase
      ...
      .eq("type", "devis_generique")
      .is("session_id", null)
      .maybeSingle();

    const docPayload = {
      formation_id,
      type: "devis_generique",
      nom_fichier: `devis_generique_${String(formation_id).slice(0, 8)}.html`,
      genere_par: "auto",
      contenu_html: html,
    ...
    if (saveErr) {
      console.error("generer-devis-generique: erreur sauvegarde:", saveErr.message);
      throw new Error("Échec de la sauvegarde du devis générique : " + saveErr.message);
    }
    ...
    console.error("generer-devis-generique: ERREUR FATALE:", msg, err instanceof Error ? err.stack : "");
    return new Response(
